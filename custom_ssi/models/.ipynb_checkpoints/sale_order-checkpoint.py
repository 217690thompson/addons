...
#             raise UserError(_(val))
            ...
            if val['product_id']:
                ...
                for i in range(0, 52 - date_from.isocalendar()[1] + 2):
                    if val['year'] == date_from.year:
                        ...
                        qty = 0
                    w_qty.append(qty)
                last_week = {str(val['product_id']): qty}  
                for j in range(0, now.isocalendar()[1] + 1):
                    if val['year'] == now.year:
                        if j == 0:
                            qty = val['w1'] + last_week[str(val['product_id'])]
                        else:
                            qty = val['w' + str(j+1)]
                    else:
                        qty = 0
                ...
                exists = self.env['sale.buy.report'].search([('product_id', '=', val['product_id'])])
                if exists:
                    for i in range(0, 53):
                        if w_qty[i] > 0:
      ...
      #         	DONT FORGET TO SET THIS BACK TO != FALSE
                    backorders = self.env['stock.picking'].sudo().search(
                      [('product_id', '=', val['product_id']), ('backorder_id', '!=', False), ('state', 'in',["confirmed","waiting"])]
                    )
                  ...
                  # REPLACE OUTGOING WITH ON PO AND PUT THAT NUMBER IN
                    ...
                    on_pos = self.env['stock.move'].sudo().search(
                      [('product_id', '=', val['product_id']), ('location_id', '=', 8), ("state","in",["assigned","confirmed","waiting"])]
                    )
                    total_po_qty = 0
                    for o in on_pos:
                    ...
                    p = self.env['product.product'].search([('id','=',val['product_id'])])
