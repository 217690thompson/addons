...
odoo.define('website_advanced_wishlist.website_advanced_wishlist', function(require) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click','.move_to_wishlist',function(e){
            ...
            last_product = $(this).attr("last");
            var e_parent = e.target.parentElement;

            if(e_parent.tagName !== "SPAN"){
                var p = e.target.getBoundingClientRect();
            }else{
                var p = e_parent.getBoundingClientRect();
            }
            
            var p_offset = window.scrollY;
            $('.move_to_wishlist_dropdown').css('top',p.top-270+p_offset+'px');
